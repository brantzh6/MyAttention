...
- `D:\code\MyAttention\ops\state\pm_control_state.json` (compact snapshot, read FIRST)
...
The PM agent runs on a **PowerShell** host. All shell commands must be
PowerShell-compatible.

**Trigger listing:** use PowerShell-native commands. Example:

```powershell
Get-ChildItem -Path 'D:\code\MyAttention\ops\triggers' -Filter 'openclaw_pm_*.json' |
  Sort-Object LastWriteTime -Descending
```

**File reads:** use `Get-Content -Raw -LiteralPath <path>`.

**Missing optional files:** check for absence explicitly instead of emitting a
failing tool command. Use `Test-Path <path>` before reading.

**Forbidden:** cmd.exe-only switches and redirection such as `/b`, `/o-d`,
`2>nul`, and bare `dir` with those flags.

A non-essential discovery command failure must not cause the overall PM run
digest status to be marked `error` when the PM decision itself succeeded.
